...
public class BaseThemePreferenceController extends AbstractPreferenceController implements
        PreferenceControllerMixin, Preference.OnPreferenceChangeListener {

    private static final String KEY_BASE_THEME = "base_theme";
    private static final String CATEGORY_BASE_THEME = "android.base_theme";

    private final MetricsFeatureProvider mMetricsFeatureProvider;
    private final OverlayManagerWrapper mOverlayService;
    private final PackageManager mPackageManager;

    public BaseThemePreferenceController(Context context) {
        this(context, ServiceManager.getService(Context.OVERLAY_SERVICE) != null
                ? new OverlayManagerWrapper() : null);
    }

    @VisibleForTesting
    BaseThemePreferenceController(Context context, OverlayManagerWrapper overlayManager) {
        super(context);
    ...
    public String getPreferenceKey() {
        return KEY_BASE_THEME;
    }

    @Override
    public boolean handlePreferenceTreeClick(Preference preference) {
        if (KEY_BASE_THEME.equals(preference.getKey())) {
    ...
    private boolean isTheme(OverlayInfo oi) {
        if (!CATEGORY_BASE_THEME.equals(oi.category)) {
            return false;
        }
        try {
